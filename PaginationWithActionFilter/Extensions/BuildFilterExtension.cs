...
namespace PaginationWithActionFilter.Extensions
{
    public class BuildFilterExtension<T>
    {
        public static IQueryable<T> ApplyFilter(IQueryable<T> query, PagingModel filter)
        {
        ...
        private static Expression BuildFilterExpression(FilterModel filter, ParameterExpression parameter)
        {
            if (filter.Filters != null && filter.Filters.Any())
            {
                ...
                {
                    var orFilters = filter.Filters.Select(f => BuildFilterExpression(f, parameter));
                    return orFilters.Aggregate(Expression.AndAlso);
                }
            }
        ...
        private static Expression<Func<T, bool>> GetAndFilterExpression(List<FilterModel> filters)
        {
        ...
        private static Expression<Func<T, bool>> GetOrFilterExpression(List<FilterModel> filters)
        {
            if (filters == null || !filters.Any())
                return null;
